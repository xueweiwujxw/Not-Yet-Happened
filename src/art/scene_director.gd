...
func stage(room: Node3D, avatar: Node3D, speaking: bool) -> void:
	player = avatar
	talking = speaking
	actor = null
	var nearest := 3.5
	for node: Node in room.find_children("*", "Node3D", true, false):
		if node.has_meta("rest_y") and node.is_visible_in_tree():
			...
			if gap < nearest:
				actor = node
				nearest = gap
	focus = avatar.position
	if actor != null:
		focus = (focus + actor.position) * 0.5
...
func tick(camera: Camera3D, delta: float) -> void:
	...
	camera.position = camera.position.lerp(aim + offset, weight)
	camera.size = lerpf(camera.size, 10.8 if close else 13.2, weight)
	camera.v_offset = lerpf(camera.v_offset, -0.85, weight)
	camera.look_at(camera.position - offset)
	if close and is_instance_valid(actor) and is_instance_valid(player) and actor.is_visible_in_tree():
		var direction := player.global_position - actor.global_position
		if Vector2(direction.x, direction.z).length() > 0.1:
			actor.rotation.y = rotate_toward(actor.rotation.y, atan2(direction.x, direction.z), delta * 1.8)
			if is_instance_valid(player_visual):
				player_visual.rotation.y = rotate_toward(player_visual.rotation.y, atan2(-direction.x, -direction.z), delta * 2.5)
